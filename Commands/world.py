...
class WorldCommand(commands.Cog):
    ...
    async def world(self, interaction: discord.Interaction, world_size: app_commands.Choice[int], world_name: str):
        ...
        asyncio.create_task(self.generate_world(world=world, interaction=interaction, message=message))

    async def generate_world(self, world, interaction, message):
        # generate noise map
        ...
        edited_embed.add_field(name="Generating world..", value="", inline=False)
        ...
        edited_embed = message.embeds[0]
        edited_embed.color = discord.Color.orange()
        edited_embed.add_field(name="Generating grass...", value="", inline=False)
        await message.edit(embed=edited_embed)
        ...
        edited_embed = message.embeds[0]
        edited_embed.color = discord.Color.orange()
        edited_embed.add_field(name="Generating trees..", value="", inline=False)
        await message.edit(embed=edited_embed)
        ...
        for block in world.get_blocks():
            if block.get_id() == 2:  # if it's grass
                ...
                if random.uniform(0, 1) < TREE_CHANCE:
                    # spawn a tree
                    await generate_tree(world=world, block=block, height=TREE_HEIGHT)
        ...
        edited_embed.add_field(name="Finished!", value="You can now play by typing `/play`", inline=False)
        await message.edit(embed=edited_embed)


async def generate_tree(world, block, height):
